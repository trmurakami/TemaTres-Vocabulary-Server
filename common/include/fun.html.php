 ...
 *
 * @param string $texto search string  
 * @param string $tipo  type search  
 * 
    ...
    if ($sql_cant > 0) {
        ...
        while ($resulta_busca = $sql->FetchRow()) {
            ...
            if ($resulta_busca["termino_preferido"]) {
                ...
                if ((in_array($resulta_busca["rr_code"], $CFG["HIDDEN_EQ"])) && (evalUserLevel($_SESSION[$_SESSION["CFGURL"]]) == 0)) {
                    ...
                    $row_result .= '<li><em><a class="' . $styleClassLink . ' ' . $styleClassLinkMetaTerm . '" title="' . LABEL_verDetalle . $resulta_busca["tema"] . '" href="' . URL_BASE . 'index.php?tema=' . $resulta_busca["tema_id"] . '">' . $resulta_busca["tema"] . '</a></em> ' . $leyendaConector . ' <a title="' . LABEL_verDetalle . $resulta_busca["tema"] . '" href="' . URL_BASE . 'index.php?tema=' . $resulta_busca["id_definitivo"] . '">' . $resulta_busca["termino_preferido"] . '</a> </li>';
                }
            } else { // es un término preferido
                $row_result .= '<li><a class="' . $styleClassLink . ' ' . $styleClassLinkMetaTerm . '"  title="' . LABEL_verDetalle . $resulta_busca["tema"] . '" href="' . URL_BASE . 'index.php?tema=' . $resulta_busca["id_definitivo"] . '">' . $resulta_busca["tema"] . '</a> ' . $leyendaTerminoLibre . '</li>';
            }
        }; //fin del while
        $row_result .= '</ul>';
        ...
        if ((strtoupper($primerTermino) !== trim(strtoupper($texto))) && (array2value("sgs", $_GET) != 'off')) {
    ...
    while ($datosTotalRelacionados = $sqlTotalRelacionados->FetchRow()) {
        ...
        switch ($datosTotalRelacionados["t_relacion"]) {
            ...
            case '4': // UF
                ...
                if ((!in_array($datosTotalRelacionados["rr_code"], $CFG["HIDDEN_EQ"])) || (evalUserLevel($_SESSION[$_SESSION["CFGURL"]]) > 0)) {
                    $iuf = ++$iuf;
...
function HTMLbodyTermino($array)
{
    ...
    // USP
    $body .= '<div id="contentComplete" class="row">';

    $body .= '<div id="content" class="col-md-8">';
    ...
    // USP links
    $body .= '<div id="content" class="col-md-4">';
    $body .= '<div id="content" class="tab-pane fade in active">';
    $body .= '<dfn><span id="strterm">Pesquisar na USP</span></dfn>';
    $body .= '<ul>';
    $body .= '<li><a target="_blank" href="http://dedalus.usp.br/F/?func=find-b&find_code=WSU&adjacent=N&local_base=ECA&x=38&y=7&filter_code_1=WLN&filter_request_1=&filter_code_2=WYR&filter_request_2=&filter_code_3=WYR&filter_request_3=&filter_code_4=WMA&filter_request_4=&filter_code_5=WBA&filter_request_5=&request=' . $array["titTema"] . '">Pesquisar no DEDALUS</a></li>';
    $body .= '<li><a target="_blank" href="https://www.buscaintegrada.usp.br/primo_library/libweb/action/search.do?fn=search&ct=search&initialSearch=true&mode=Advanced&tab=default_tab&indx=1&dum=true&srt=rank&vid=USP&frbg=&tb=t&vl%284708870UI0%29=sub&vl%284708870UI0%29=title&vl%284708870UI0%29=any&vl%281UIStartWith0%29=exact&vl%284708294UI0%29=AND&vl%284708867UI1%29=any&vl%284708867UI1%29=title&vl%284708867UI1%29=any&vl%281UIStartWith1%29=contains&vl%28freeText1%29=&vl%284708296UI1%29=AND&vl%284708875UI2%29=any&vl%284708875UI2%29=title&vl%284708875UI2%29=any&vl%281UIStartWith2%29=contains&vl%28freeText2%29=&vl%284708296UI2%29=AND&vl%284708880UI3%29=all_items&vl%284708883UI4%29=all_items&vl%28D4708300UI5%29=all_items&vl%284708301UI6%29=00&vl%284708302UI6%29=00&vl%284708303UI6%29=&vl%284708304UI6%29=00&vl%284708305UI6%29=00&vl%284708306UI6%29=&Submit=Buscar&vl%28freeText0%29=' . $array["titTema"] . '">Pesquisar no Portal de Busca Integrada USP</a></li>';
    $body .= '<li><a target="_blank" href="https://repositorio.usp.br/result.php?filter[]=about:%22' . $array["titTema"] . '%22">Pesquisar no Repositório da Produção USP</a></li>';
    $body .= '</ul>';
    $body .= '</div>';
    $body .= '</div>';
    ...
    if ($i_profundidad == 1) {
        $body .= HTMLcloudTerms($middle_deep, 10, $array["tema_id"]);
    }
...
function HTMLmainMenu()
{
    ...
    if ($evalUserLevel == 1) {
        ...
        $row .= '<li><a title="' . ucfirst(LABEL_import) . '" href="admin.php?doAdmin=import" >' . ucfirst(LABEL_import) . '</a></li>';
...
function HTMLtargetTerms($tema_id)
{
    ...
    if (SQLcount($sql) > 0) {
        ...
        while ($array = $sql->FetchRow()) {
            ...
            $rows .= (($_GET["taskEdit"] == 'checkDateTermsTargetVocabulary') && ($_GET["tterm_id"] == $array["tterm_id"]) && (evalUserLevel($_SESSION[$_SESSION["CFGURL"]]) > 0)) ? HTMLcheckTargetTerm($array) : '';
            $rows .= '</li>';
        }
        $rows .= '</ul>';
    } else {
...
function HTMLtermMetadata($arrayTerm, $arrayCantRelaciones)
{
    ...
    if ((evalUserLevel($_SESSION[$_SESSION["CFGURL"]]) == 1) && ($arrayCantRelaciones["cantNT"] > 0)) {
...
function HTMLjsInclude()
{
    ...
    if ((evalUserLevel($_SESSION[$_SESSION["CFGURL"]]) == 1) & (array2value("doAdmin", $_GET) == 'export')) {
...
function HTMLsummary()
{
    ...
    if ($_SESSION["CFGPublisher"]) {
        $rows .= '<tr><th>' . ucfirst(LABEL_Publisher) . '</th><td>' . $_SESSION["CFGPublisher"] . '</td></tr>';
    }
    ...
    if (CFG_ENABLE_SPARQL == 1) {
        $rows .= '<tr><th>' . ucfirst(LABEL_SPARQLEndpoint) . '</th><td><a href="' . $_SESSION["CFGURL"] . 'sparql.php" title="' . LABEL_SPARQLEndpoint . '">' . $_SESSION["CFGURL"] . 'sparql.php</a></td></tr>';
    }
 ...
 *
 * @param array  $metadata  term metadata  
 * @param string $note_type type note  
 * @param string $format    output format  
 * @param string $editFlag  flag to enable/disable edit
 * 
 * @return return html content
 */
...
function HTMLcopyClick($targt_div, $array_flags)
{
    ...
    if (($array_flags["isMetaTerm"] == 1) || ($array_flags["isValidTerm"] == 0) || ($array_flags["copy_click"] == 0)) {
        return;
    }
...
function HTMLsource4note($note_id)
{
    ...
    $src_link = (strlen($array['src_url']) > 5) ? ' <a href="' . $array['src_url'] . '" target="_blank" title="' . ucfirst(LABEL_Detalle) . ' ' . $array["src_alias"] . '">' . $array['src_url'] . '</a>' : '';

    $rows = '<div class="src4note">';
    $rows .= ucfirst(LABEL_src_note) . ': <a data-toggle="collapse" href="#src4note' . $note_id . '_' . $array["src_id"] . '" href="#">' . $array["src_alias"] . '</a>';
    $rows .= '	<p class="collapse" id="src4note' . $note_id . '_' . $array["src_id"] . '">' . $array["src_note"] . $src_link . '</p>';
    $rows .= '</div>';
    return $rows;
}
...
function TXTsource4note($note_id)
{
    ...
    return ucfirst(LABEL_src_note) . ': [' . $array["src_alias"] . '] ' . $array["src_note"] . ' ' . $array['src_url'];
}


function HTMLSources4vocab()
{
    ...
    while ($array = $sql->FetchRow()) {
        $src_link = (strlen($array['src_url']) > 5) ? ' <a href="' . $array['src_url'] . '" target="_blank" title="' . ucfirst(LABEL_Detalle) . ' ' . $array["src_alias"] . '">' . $array['src_url'] . '</a>' : '';
        if ($array["src_cant"] > 0) {
            $rows .=  '<li>[<a href="index.php?src_id=' . $array["src_id"] . '" title="' . $array["src_alias"] . ': ' . $array["src_cant"] . ' ' . ucfirst(LABEL_Terminos) . '"> ' . $array["src_alias"] . '</a>] ' . $array["src_note"] . ' ' . $src_link . '</li>';
        } else {
            $rows .=  '<li>[' . $array["src_alias"] . '] ' . $array["src_note"] . ' ' . $src_link . '</li>';
        }
    }
    $rows .= '</ul>';
    $rows .= '</div><br>'; //end div
...
function HTMLterms4source($src_id)
{

    $sql = SQLterms4src($src_id);

    $ARRAYsrc = ARRAYsource($src_id);
    ...
    while ($array = $sql->FetchRow()) {
        $rows .= '<li>' . HTMLlinkTerm(array("tema_id" => $array["tema_id"], "tema" => $array["tema"])) . '</li>';
    };
    $rows .= '   </ul>';
    $rows .= '</div>';
...
function HTMLtermsXcluster($t_relacion)
{
    ...
    [ Chartist.plugins.ctAxisTitle({ axisX: { axisTitle: "' . ucfirst($label_x) . '", axisClass: "ct-axis-title", offset: { x:
